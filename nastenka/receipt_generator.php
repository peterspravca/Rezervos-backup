...
$page_title = $draft_data ? 'Úprava príjmového dokladu' : 'Nový príjmový doklad';
?>
<!DOCTYPE html>
<html lang="sk">
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .toolbar-input {
            ...
            border-radius: 12px; color: var(--text-primary); padding: 0.5rem; outline: none; width: 120px;
        ...
        .printable-document {
            width: 210mm; min-height: 148mm; background: white;
        ...
        [contenteditable="true"]:focus { background: rgba(8,145,178,0.05); outline: none; border-radius: 4px; }
        [contenteditable="true"]:hover { background: rgba(8,145,178,0.03); border-radius: 4px; }

        @media print {
            @page { size: auto; margin: 0mm; }
            * { -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
            .app-toolbar { display: none !important; }
            .app-main { margin-top: 0; padding: 20mm 0; background: white; overflow: visible; }
            .printable-document { box-shadow: none; margin: 0; padding: 15mm; width: 210mm; height: 148mm; transform: none !important; border: 1px solid #ccc; }
...
<body>
    ...
    <header class="app-toolbar">
        <div class="toolbar-left">
            ...
            <div class="nav-text-mobile" style="font-weight: 700; font-size: 1.1rem; color: var(--accent); letter-spacing: -0.01em;">Príjmový doklad</div>
        </div>
        ...
        <div class="toolbar-center">
            <div style="display: flex; align-items: center; gap: 0.5rem; background: rgba(0,0,0,0.2); padding: 4px 8px; border-radius: 12px; border: 1px solid var(--border);">
                <span class="hide-mobile" style="font-size: 0.8rem; color: var(--text-muted);">SUMA:</span>
                <input type="number" step="0.01" class="toolbar-input" id="amount_input" value="100.00" style="background:transparent; border:none; outline:none; color:white; width:90px; text-align:center; font-weight:800; font-size:1.1rem;">
    ...
    <main class="app-main" <?= $lead_id === 0 ? 'style="margin-top: 110px;"' : '' ?>>
        <div id="receipt_document" class="printable-document">
            
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 25px; border-bottom: 3px solid #0891b2; padding-bottom: 15px;">
                <div style="width: 150px;">
                    <img src="vueto_logo.png" style="max-height: 45px; max-width: 100%; object-fit: contain;">
                </div>
                <div style="text-align: right;">
                    <h1 style="color: #0891b2; margin: 0; font-size: 1.8rem; font-weight: 800; text-transform: uppercase;">Príjmový pokladničný doklad</h1>
                    <div style="font-size: 1.1rem; font-weight: 700; color: #333; margin-top: 5px;">
                        číslo: <span contenteditable="true" id="receipt_number" style="border-bottom: 1px dotted #ccc; outline: none; min-width: 100px; display: inline-block; color: #0891b2;">PPD-<?= date('Y') ?>001</span>
                    </div>
                </div>
            </div>
            ...
            <div style="display: flex; gap: 30px; margin-bottom: 25px;">
                <div style="flex: 1; border: 1px solid #f1f5f9; border-radius: 8px; padding: 12px;">
                    <div style="background: #ecfeff; color: #0e7490; padding: 4px 10px; font-weight: 800; font-size: 0.7rem; text-transform: uppercase; letter-spacing: 1px; border-radius: 4px; margin-bottom: 8px; display: inline-block;">
                        Príjemca (Firma)
                    </div>
                    <div style="padding: 0 10px; line-height: 1.4; font-size: 0.8rem;">
                        <div contenteditable="true" style="font-weight: 800; font-size: 0.9rem; margin-bottom: 2px;">CONTRACTOR GROUPS s.r.o.</div>
                ...
                <div style="flex: 1; border: 1px solid #f1f5f9; border-radius: 8px; padding: 12px;">
                    ...
                    <div style="padding: 0 10px; line-height: 1.4; font-size: 0.8rem;">
                        <div contenteditable="true" style="font-weight: 800; font-size: 0.9rem; margin-bottom: 2px; color: #0891b2;"><?= $client ? htmlspecialchars($client['name']) : 'Meno klienta' ?></div>
                        <?php if($client && !empty($client['company'])): ?>
                            <div contenteditable="true"><?= htmlspecialchars($client['company']) ?></div>
                        <?php endif; ?>
                        <div contenteditable="true">
                            <?= ($client && $client['address']) ? htmlspecialchars($client['address']) : 'Adresa / Sídlo' ?>
            ...
            <table style="width: 100%; border-collapse: collapse; margin-bottom: 25px;">
                <tr>
                    <td style="padding: 12px; border: 1px solid #f1f5f9; background: #f8fafc; width: 140px; font-weight: 700; font-size: 0.8rem; text-transform: uppercase; color: #64748b;">Suma €</td>
                    <td style="padding: 12px; border: 1px solid #f1f5f9; font-size: 1.5rem; font-weight: 800; color: #0891b2;">
                        <span id="display_amount">100,00</span> €
                    </td>
                </tr>
                <tr>
                    <td style="padding: 12px; border: 1px solid #f1f5f9; background: #f8fafc; font-weight: 700; font-size: 0.8rem; text-transform: uppercase; color: #64748b;">Suma slovom</td>
                    <td style="padding: 12px; border: 1px solid #f1f5f9; font-size: 0.85rem; font-style: italic;">
                        <div contenteditable="true" id="amount_word" style="outline: none;">Jednosto eur</div>
                    </td>
                </tr>
                <tr>
                    <td style="padding: 12px; border: 1px solid #f1f5f9; background: #f8fafc; font-weight: 700; font-size: 0.8rem; text-transform: uppercase; color: #64748b;">Účel platby</td>
                    <td style="padding: 12px; border: 1px solid #f1f5f9; font-size: 0.85rem;">
                        <div contenteditable="true" id="receipt_purpose" style="outline: none;">Úhrada za dodávku tovaru a služieb podľa zmluvy/ponuky.</div>
                    </td>
                </tr>
            </table>

            <div style="display: flex; gap: 40px; font-size: 0.85rem; margin-bottom: 40px; padding: 0 10px;">
                ...
                <div>Dňa: <input type="text" id="receipt_date" value="<?= date('d.m.Y') ?>" style="border: none; border-bottom: 1px dotted #cbd5e1; outline: none; font-weight:600; font-family: inherit; font-size: inherit; width: 90px; background: transparent;"></div>
            </div>

            <div style="display: flex; justify-content: space-between; gap: 50px;">
                <div style="flex: 1;">
                    <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 6px; color: #475569; font-size: 0.85rem;">
                        <i class="ti ti-user" style="color: #0891b2; font-size: 1.1rem;"></i>
                        <span contenteditable="true" style="outline:none; font-weight: 600;"><?= htmlspecialchars($user['full_name']) ?></span>
                    </div>

                    <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 6px; color: #475569; font-size: 0.85rem;">
                        <i class="ti ti-mail" style="color: #0891b2; font-size: 1.1rem;"></i>
                ...
                <div style="flex: 1; text-align: center;">
                    ...
                    <div id="sig_trigger" class="sig-preview-trigger" onclick="openSignatureModalGlobal('signature_data', 'sig_preview_img', 'sig_placeholder_modal')">
                        ...
                        <img src="" id="sig_preview_img" class="sig-preview-img-modal" alt="Podpis platiteľa" style="max-height: 80px; display: none;">
                    </div>
                    <div style="font-weight: 800; font-size: 0.75rem; text-transform: uppercase; color: #666; margin-top: 5px;">Odovzdal (Platiteľ)</div>
                </div>
            </div>

            <div style="margin-top: 40px; text-align: center; border-top: 1px solid #f1f5f9; padding-top: 15px; color: #94a3b8; font-size: 0.7rem;">
                Tento doklad slúži ako potvrdenie o prijatí platby v hotovosti. Vygenerované v CRM.
            </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const display_amount = document.getElementById('display_amount');

            if (leadId && !editToken) {
                fetch(`ajax_get_doc_info.php?lead_id=${leadId}&type=receipt`)
                    .then(res => res.json())
                    .then(data => {
                        if (data.success) {
                            ...
                            if (data.already_exists) {
                                document.getElementById('duplicateWarningMsg').innerHTML = 
                                    `Pozor, tomuto klientovi už bol dňa ${new Date(data.existing_docs[0].created_at).toLocaleDateString('sk-SK')} vystavený <strong>Príjmový doklad (${data.existing_docs[0].doc_number})</strong>.`;
            ...
            if(amount_input) {
                amount_input.addEventListener('input', function() {
                    display_amount.textContent = formatNumber(this.value || 0);
                });
            }

            if (draftData) {
                if (draftData.number) document.getElementById('receipt_number').innerText = draftData.number;
                if (draftData.amount) {
                    amount_input.value = draftData.amount;
                    amount_input.dispatchEvent(new Event('input'));
                }
                if (draftData.amount_word) document.getElementById('amount_word').innerText = draftData.amount_word;
                if (draftData.purpose) document.getElementById('receipt_purpose').innerText = draftData.purpose;
                if (draftData.date) document.getElementById('receipt_date').value = draftData.date;
            ...
            window.saveDocument = function(showUrl = false, status = 'sent', sendEmail = false) {
                ...
                try {
                    ...
                    btn.innerHTML = '<i class="ti ti-loader-2 rotate"></i> Ukladám...';
                    const editorData = {
                        number: document.getElementById('receipt_number').innerText,
                        date: document.getElementById('receipt_date').value,
                        amount: amount_input.value,
                        amount_word: document.getElementById('amount_word').innerText,
                        purpose: document.getElementById('receipt_purpose').innerText,
                        signature_data: document.getElementById('signature_data').value
                    };

                    const docClone = document.getElementById('receipt_document').cloneNode(true);
                    docClone.querySelectorAll('.remove-row, .btn').forEach(el => el.remove());
                    docClone.querySelectorAll('[contenteditable]').forEach(el => el.removeAttribute('contenteditable'));
                    const originalElements = document.getElementById('receipt_document').querySelectorAll('input, select');
                    ...
                    data.append('title', 'Príjmový doklad ' + editorData.number);
    ...
    <div id="shareModal" class="modal-overlay">
        <div class="modal-box" style="text-align: center;">
            ...
            <div style="display: flex; gap: 10px; justify-content: center; margin-top: 2rem;">
                ...
                <a href="#" id="open_link_btn" target="_blank" class="btn btn-primary" style="min-width:    <?php include __DIR__ . '/partials/signature_modal.php'; ?>
</body>
</html>
