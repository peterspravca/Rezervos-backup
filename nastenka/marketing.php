...
// ── UTM Leads Logic ──
$utm_leads = $pdo->query("SELECT id, name, created_at, utm_source, utm_medium, utm_campaign, page_url FROM leads WHERE utm_source != '' OR utm_medium != '' OR utm_campaign != '' OR page_url != '' ORDER BY created_at DESC LIMIT 100")->fetchAll();
...
<style>
    .marketing-tabs { display: flex; gap: 10px; margin-bottom: 2rem; border-bottom: 1px solid var(--border); padding-bottom: 10px; }
    .m-tab { padding: 10px 20px; border-radius: 8px; cursor: pointer; font-weight: 600; color: var(--text-muted); transition: all 0.2s; }
    .m-tab:hover { background: var(--bg-hover); color: var(--text-primary); }
...
<div class="marketing-tabs flex-responsive">
    <div class="m-tab active" onclick="switchMarketingTab('emails', this)"><i class="ti ti-mail"></i> E-mailové kampane</div>
    <div class="m-tab" onclick="switchMarketingTab('utm', this)"><i class="ti ti-chart-bar"></i> Zdroje dopytov (UTM)</div>
</div>

<!-- EMAILS TAB -->
<div id="m-tab-emails" class="m-tab-content">
    <div class="flex justify-between items-center mb-2">
        <h2 class="m-0"><i class="ti ti-mail-forward"></i> Hromadné e-maily</h2>
        <div class="flex flex-responsive gap-1">
            <a href="marketing.php?type=all&gender=" class="btn <?= ($target_type==='all' && empty($f_gender))?'btn-primary':'btn-secondary' ?> btn-sm">Všetci</a>
            <a href="marketing.php?type=all&gender=male" class="btn <?= $f_gender==='male'?'btn-primary':'btn-secondary' ?> btn-sm"><i class="ti ti-user-bolt icon-male" style="margin-right:4px;"></i> Muži</a>
            <a href="marketing.php?type=all&gender=female" class="btn <?= $f_gender==='female'?'btn-primary':'btn-secondary' ?> btn-sm"><i class="ti ti-user-heart icon-female" style="margin-right:4px;"></i> Ženy</a>
            <a href="marketing.php?type=all&gender=other" class="btn <?= $f_gender==='other'?'btn-primary':'btn-secondary' ?> btn-sm"><i class="ti ti-building-community icon-firm" style="margin-right:4px;"></i> Firmy</a>
            <a href="marketing.php?type=nameday&gender=<?= $f_gender ?>" class="btn <?= $target_type==='nameday'?'btn-primary':'btn-secondary' ?> btn-sm">Dnešné meniny</a>
            <a href="marketing.php?type=customers&gender=<?= $f_gender ?>" class="btn <?= $target_type==='customers'?'btn-primary':'btn-secondary' ?> btn-sm">Zákazníci</a>
        </div>
    </div>
                        ...
                        <tbody>
                            ...
                            <tr id="recipient-<?= $l['id'] ?>" style="border-bottom: 1px solid rgba(255,255,255,0.03);">
                                ...
                                <td style="text-align: right; padding: 0.65rem 1rem;">
                                    <button type="button" class="btn btn-sm action-btn" style="background:transparent; color: #ef4444; opacity: 0.8; padding: 4px; transition: opacity 0.2s;" onclick="excludeRecipient(<?= $l['id'] ?>)" title="Vyradiť">
...
<!-- UTM TAB -->
<div id="m-tab-utm" class="m-tab-content" style="display:none;">
    <div class="flex justify-between items-center mb-2">
        <h2 class="m-0"><i class="ti ti-chart-bar"></i> Sledovanie zdrojov (UTM)</h2>
    </div>
    
    <div class="card">
        <div class="card-header">
            <div class="card-title">Prehľad dopytov podľa marketingu</div>
            <div class="card-subtitle">Tu vidíte, z akých kampaní a zdrojov prišli vaši zákazníci.</div>
        </div>
        <div class="p-0">
            <?php if(empty($utm_leads)): ?>
                <p class="text-muted text-center py-4">Zatiaľ žiadne dopyty s UTM parametrami.</p>
            <?php else: ?>
                <table class="data-table">
                    <thead>
                        <tr>
                            <th>Dátum</th>
                            <th>Meno</th>
                            <th>Zdroj (Source) / Médium</th>
                            <th>Kampaň</th>
                            <th>Vstupná stránka</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php foreach($utm_leads as $u): ?>
                        <tr>
                            <td><?= date('d.m.Y H:i', strtotime($u['created_at'])) ?></td>
                            <td><a href="contact.php?id=<?= $u['id'] ?>" class="text-bold"><?= htmlspecialchars($u['name']) ?></a></td>
                            <td>
                                <span class="status-badge" style="background:rgba(99,102,241,0.1); color:var(--accent);"><?= htmlspecialchars($u['utm_source'] ?: '-') ?></span>
                                <span class="text-muted" style="font-size:0.8rem;"><?= htmlspecialchars($u['utm_medium'] ?: '-') ?></span>
                            </td>
                            <td><span class="text-bold"><?= htmlspecialchars($u['utm_campaign'] ?: '-') ?></span></td>
                            <td><span class="text-muted" style="font-size:0.75rem;"><?= htmlspecialchars($u['page_url'] ?: '-') ?></span></td>
                        </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
...
function switchMarketingTab(tabId, btn) {
    ...
    document.getElementById('m-tab-' + tabId).style.display = 'block';
...
async function openPreview() {
    ...
    if (tplId === 'none') {
        iframe.contentDocument.body.innerHTML = `<div style="font-family: sans-serif; padding: 40px;"><h2>${subject}</h2><hr>${body.replace(/\n/g, '<br>')}</div>`;
    } else {
...
function submitMainForm() {
    const mainForm = document.getElementById('campaign-form');
    const count = allRecipients.length;
    if (count <= 0) { alert("Zoznam príjemcov je prázdny."); return; }
    if (confirm(`Spustiť hromadné odosielanie pre ${count} príjemcov?`)) mainForm.submit();
}
