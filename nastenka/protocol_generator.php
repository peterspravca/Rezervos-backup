...
$page_title = $draft_data ? 'Úprava preberacieho protokolu' : 'Nový preberací protokol';
?>
<!DOCTYPE html>
<html lang="sk">
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .toolbar-select {
            ...
            border-radius: 12px; color: var(--text-primary); padding: 0.5rem; outline: none; cursor: pointer;
        }
        ...
        [contenteditable="true"]:focus { background: rgba(79, 70, 229, 0.05); outline: none; border-radius: 4px; }
        [contenteditable="true"]:hover { background: rgba(79, 70, 229, 0.03); border-radius: 4px; }
...
<body>
    ...
    <header class="app-toolbar">
        <div class="toolbar-left">
            ...
            <div class="nav-text-mobile" style="font-weight: 700; font-size: 1.1rem; color: var(--accent); letter-spacing: -0.01em;">Preberací protokol</div>
        </div>

        <div class="toolbar-center">
            <select class="toolbar-select" id="condition_toggle" style="background: rgba(0,0,0,0.2); border: 1px solid var(--border); border-radius: 10px; color: white; padding: 0.4rem 0.6rem;">
                <option value="none">Bez závad</option>
                <option value="defects">So závadami</option>
            </select>
            <button class="btn btn-secondary" id="add_attachment_btn">
                <i class="ti ti-paperclip"></i> <span class="hide-mobile">Príloha</span>
            </button>
        </div>
    ...
    <div class="client-warning-banner" id="client_warning">
        ...
        <div style="display: flex; gap: 10px;">
            <a href="board.php" class="btn btn-sm btn-secondary" style="height: 30px; font-size: 0.75rem;">Vybrať z CRM</a>
            <button class="btn btn-sm btn-primary" onclick="showQuickAddModal()" style="height: 30px; font-size: 0.75rem; background: #2563eb;">Vybrať alebo pridať klienta</button>
        </div>
    </div>
    <?php endif; ?>

    <main class="app-main" <?= $lead_id === 0 ? 'style="margin-top: 110px;"' : '' ?>>
        <div id="protocol_document" class="printable-document">
            ...
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 25px;">
                ...
                <div style="text-align: right;">
                    <h1 style="color: #4f46e5; margin: 0; font-size: 2.25rem; font-weight: 800; text-transform: uppercase;">Preberací protokol</h1>
                    <div style="font-size: 1.1rem; color: #666; margin-top: 2px;">
                        k zmluve č.: <span id="protocol_number" contenteditable="true" style="border-bottom: 1px dotted #ccc; outline: none; min-width: 80px; display: inline-block;">PON-<?= date('Y') ?>001</span>
                    </div>
                </div>
            </div>
            ...
            <div style="display: flex; gap: 30px; margin-bottom: 30px;">
                <div style="flex: 1; border: 1px solid #f1f5f9; border-radius: 8px; padding: 12px;">
                    <div style="background: #eef2ff; color: #312e81; padding: 4px 10px; font-weight: 800; font-size: 0.75rem; text-transform: uppercase; letter-spacing: 1px; border-radius: 4px; margin-bottom: 10px;">
                        Zhotoviteľ
                    </div>
                ...
                <div style="flex: 1; border: 1px solid #f1f5f9; border-radius: 8px; padding: 12px;">
                    ...
                    <div style="padding: 0 10px; line-height: 1.5; font-size: 0.85rem;">
                        <div contenteditable="true" style="font-weight: 800; font-size: 1rem; margin-bottom: 2px; outline: none; color: #4f46e5;"><?= $client ? htmlspecialchars($client['name']) : 'Meno klienta' ?></div>
            ...
            <div style="margin-bottom: 25px; border-bottom: 1px solid #f1f5f9; padding-bottom: 15px;">
                <div style="font-size: 0.75rem; font-weight: 800; color: #94a3b8; text-transform: uppercase; margin-bottom: 5px;">Predmet odovzdania:</div>
                <div contenteditable="true" style="outline: none; line-height: 1.6; font-size: 0.95rem;">
                    Dielo zhotovené podľa vyššie uvedenej objednávky v požadovanom rozsahu a kvalite.
                </div>
            </div>

            <div style="margin-bottom: 30px;">
                <div id="status_message" style="background: #f0fdf4; color: #166534; padding: 15px; border-radius: 12px; font-size: 0.9rem; border: 1px solid #dcfce7;">
                    <i class="ti ti-check" style="margin-right: 8px;"></i> Objednávateľ vyhlasuje, že dielo preberá <strong>bez závad a nedorobkov</strong>.
                </div>
                
                <div id="defects_container" style="display: none;">
                    <div style="background: #fff1f2; color: #991b1b; padding: 12px; border-radius: 12px 12px 0 0; font-size: 0.9rem; border: 1px solid #fecaca; border-bottom: none; display: flex; justify-content: space-between; align-items: center;">
                        <span><i class="ti ti-alert-circle" style="margin-right: 8px;"></i> Zistené <strong>závady a nedorobky</strong>:</span>
                        <button class="btn btn-secondary btn-sm" id="add_defect_btn" style="height:28px; padding:0 10px; font-size:0.7rem;">+ Pridať závadu</button>
                    </div>
                    <table style="width: 100%; border-collapse: collapse; font-size: 0.85rem; border: 1px solid #fecaca;">
                        <thead>
                            <tr style="background: #fff1f2;">
                                <th style="padding: 10px; text-align: left; border: 1px solid #fecaca; color: #991b1b;">Popis vady / nedorobku</th>
                                <th style="padding: 10px; text-align: center; border: 1px solid #fecaca; width: 120px; color: #991b1b;">Termín odstr.</th>
                            </tr>
                        </thead>
                        <tbody id="defects_body">
                            <tr>
                                <td style="padding: 12px; border: 1px solid #fecaca;"><div contenteditable="true" style="outline: none;">Popis vady...</div></td>
                                <td style="padding: 12px; border: 1px solid #fecaca; text-align: center;"><div contenteditable="true" style="outline: none;"><?= date('d.m.Y', strtotime('+7 days')) ?></div></td>
                            </tr>
                        </tbody>
                    </table>
                </div>
            ...
            <div style="margin-bottom: 30px;">
                <div style="font-size: 0.75rem; font-weight: 800; color: #94a3b8; text-transform: uppercase;">Prílohy a odovzdané kľúče:</div>
                <ul id="attachments_list" style="margin: 10px 0 0; padding-left: 20px; font-size: 0.9rem; line-height: 1.8;">
                    <li style="position: relative;">
                        <div contenteditable="true" style="display: inline-block; outline: none; min-width: 250px;"></div>
                        <button class="remove-row" style="position:absolute; left:-35px; top:2px; border:none; background:none; color:#f87171; cursor:pointer; font-size: 1.25rem; opacity: 0.3;">×</button>
                    </li>
                    <li style="position: relative;">
                        <div contenteditable="true" style="display: inline-block; outline: none; min-width: 250px;"></div>
                        <button class="remove-row" style="position:absolute; left:-35px; top:2px; border:none; background:none; color:#f87171; cursor:pointer; font-size: 1.25rem; opacity: 0.3;">×</button>
            ...
            <div style="display: flex; gap: 40px; font-size: 0.9rem; margin-bottom: 50px;">
                <div>V mieste: <span contenteditable="true" style="border-bottom: 1px dotted #cbd5e1; outline: none; font-weight: 600;">Žilina</span></div>
                <div>Dňa: <input type="text" id="protocol_date" value="<?= date('d.m.Y') ?>" style="border:none; border-bottom: 1px dotted #cbd5e1; outline: none; font-weight: 600; font-family: inherit; font-size: 0.9rem; width: 80px;"></div>
            </div>

            <div style="display: flex; justify-content: space-between; gap: 40px;">
                <div style="flex: 1;">
                    <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 10px; color: #475569; font-size: 0.9rem;">
                        <i class="ti ti-user" style="color: #4f46e5; font-size: 1.2rem;"></i>
                        <span contenteditable="true" style="outline:none; font-weight: 600;"><?= htmlspecialchars($user['full_name']) ?></span>
                    </div>

                    <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 10px; color: #475569; font-size: 0.9rem;">
                        <i class="ti ti-mail" style="color: #4f46e5; font-size: 1.2rem;"></i>
                ...
                <div style="flex: 1; text-align: center;">
                    ...
                    <div style="font-size: 0.75rem; font-weight: 700; text-transform: uppercase; margin-top: 5px;">Objednávateľ (Odberateľ)</div>
                </div>
            </div>

            <div style="margin-top: 50px; border-top: 1px solid #f1f5f9; padding-top: 20px; font-size: 0.75rem; color: #94a3b8; text-align: center;">
                Protokol potvrdzuje ukončenie a prevzatie diela. Vygenerované v CRM.
            </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const defects_body = document.getElementById('defects_body');
            const attachments_list = document.getElementById('attachments_list');

            if (leadId && !editToken) {
                fetch(`ajax_get_doc_info.php?lead_id=${leadId}&type=protocol`)
                    .then(res => res.json())
                    .then(data => {
                        if (data.success) {
                            const numEl = document.getElementById('protocol_number');
                            if (numEl) numEl.innerText = data.next_number;
                            if (data.already_exists) {
                                document.getElementById('duplicateWarningMsg').innerHTML = 
                                    `Pozor, tomuto klientovi už bol dňa ${new Date(data.existing_docs[0].created_at).toLocaleDateString('sk-SK')} odoslaný <strong>Preberací protokol (${data.existing_docs[0].doc_number})</strong>.`;
            ...
            document.getElementById('condition_toggle').addEventListener('change', function() {
                const isDefects = this.value === 'defects';
                document.getElementById('defects_container').style.display = isDefects ? 'block' : 'none';
                document.getElementById('status_message').style.display = isDefects ? 'none' : 'block';
            });

            window.addDefectRow = function(data = null) {
                const tr = document.createElement('tr');
                tr.className = 'item-row';
                tr.innerHTML = `
                    <td style="padding: 12px; border: 1px solid #fecaca; position: relative;">
                        <div class="item-desc" contenteditable="true" style="outline: none;">${data ? data.description : 'Popis vady...'}</div>
                    ...
                    <td style="padding: 12px; border: 1px solid #fecaca; text-align: center;"><div class="qty" contenteditable="true" style="outline: none;">${data ? data.deadline : '-'}</div></td>
                `;
                defects_body.appendChild(tr);
            };

            window.addAttachmentRow = function(text = '') {
                const li = document.createElement('li');
                li.style.position = 'relative';
                li.innerHTML = `
                    <div contenteditable="true" style="display: inline-block; outline: none; min-width: 250px;">${text || 'Nová príloha...'}</div>
                    <button class="remove-row" style="position:absolute; left:-35px; top:2px; border:none; background:none; color:#ef4444; cursor:pointer; opacity: 0.3;"><i class="ti ti-trash"></i></button>
                `;
                attachments_list.appendChild(li);
            };

            if(document.getElementById('add_defect_btn')) document.getElementById('add_defect_btn').addEventListener('click', () => window.addDefectRow());
            if(document.getElementById('add_attachment_btn')) document.getElementById('add_attachment_btn').addEventListener('click', () => window.addAttachmentRow());
            ...
            if (draftData) {
                if (draftData.number) document.getElementById('protocol_number').innerText = draftData.number;
                if (draftData.date) document.getElementById('protocol_date').value = draftData.date;
                ...
                if (draftData.items) {
                    defects_body.innerHTML = '';
                    draftData.items.forEach(item => window.addDefectRow({ description: item.desc, deadline: item.deadline }));
                }
                if (draftData.attachments) {
                    attachments_list.innerHTML = '';
                    draftData.attachments.forEach(att => window.addAttachmentRow(att));
                }
            }
            ...
            window.saveDocument = function(showUrl = false, status = 'sent', sendEmail = false) {
                ...
                try {
                    ...
                    btn.innerHTML = '<i class="ti ti-loader-2 rotate"></i> Ukladám...';
                    const editorData = {
                        number: document.getElementById('protocol_number').innerText,
                        date: document.getElementById('protocol_date').value,
                        condition: document.getElementById('condition_toggle').value,
                        signature_data: document.getElementById('signature_data').value,
                        items: [],
                        attachments: []
                    };
                    if (document.getElementById('condition_toggle').value === 'defects') {
                        document.querySelectorAll('#defects_body tr').forEach(tr => {
                            editorData.items.push({ desc: tr.querySelector('.item-desc').innerText, deadline: tr.querySelector('.qty').innerText });
                        });
                    }
                    document.querySelectorAll('#attachments_list li').forEach(li => {
                        const text = li.querySelector('div').innerText.trim();
                        if (text) editorData.attachments.push(text);
                    });

                    const docClone = document.getElementById('protocol_document').cloneNode(true);
                    ...
                    const sigCanvas = document.getElementById('sig_receiver');
                    if (sigCanvas) data.append('signature_receiver', sigCanvas.toDataURL());
    ...
    <?php include __DIR__ . '/partials/signature_modal.php'; ?>
                        ...
                        <td style="text-align:right;"><button class="remove-row" style="background:none; border:none; color:#f87171; cursor:pointer;"><i class="ti ti-trash"></i></button></td>
                    `;
                    defects_body.appendChild(tr);
                });
            }
            if (draftData.attachments && draftData.attachments.length > 0) {
                ...
                draftData.attachments.forEach(att => {
                    const li = document.createElement('li');
                    li.style.display = 'flex'; li.style.justifyContent = 'space-between'; li.style.alignItems = 'center'; li.style.marginBottom = '5px';
                    li.innerHTML = `
                        <div contenteditable="true" style="outline:none; flex:1; border-bottom:1px solid #eee;">${att}</div>
                        <button class="remove-row" style="background:none; border:none; color:#f87171; cursor:pointer; margin-left:10px;"><i class="ti ti-trash"></i></button>
                    `;
                    attachments_list.appendChild(li);
                });
            }
        ...
        window.saveDocument = function(showUrl = false, status = 'sent') {
            const leadId = '<?= $lead_id ?>';
            if (!leadId || leadId === '0') {
                alert("Vyberte prosím klienta z CRM.");
                return;
            }
            ...
            const originalContent = btn.innerHTML;
            btn.disabled = true;
            btn.innerHTML = '<i class="ti ti-loader-2 ti-spin"></i> Ukladám...';
            ...
            // Collect defects if "defects" mode is on
            if (document.getElementById('condition_toggle').value === 'defects') {
                document.querySelectorAll('#defects_body tr').forEach(tr => {
                    editorData.items.push({
                        desc: tr.querySelector('.item-desc').innerText,
                        deadline: tr.querySelector('.qty').innerText
                    });
                });
            }
            ...
            .then(res => {
                if (res.success) {
                    if (status === 'draft') {
                        ...
                        showShareModal(null, "Preberací protokol bol úspešne vložený do CRM.");
                    }
                    if (!editToken) editToken = res.token;
                } else {
                    alert("Chyba: " + res.error);
    ...
    <div id="quickAddModal" class="modal-overlay">
        <div class="modal-box">
            <div class="modal-header">
                <div class="modal-title"><i class="ti ti-users"></i> Vybrať alebo pridať klienta</div>
                <button class="modal-close" onclick="closeQuickAddModal()">✕</button>
            </div>
            
            <!-- Tabs -->
            <div class="tabs">
                <button id="tab-search-btn" class="tab-btn active" onclick="switchPickerTab('search')"><i class="ti ti-search"></i> Hľadať v CRM</button>
                <button id="tab-add-btn" class="tab-btn" onclick="switchPickerTab('add')"><i class="ti ti-plus"></i> Rýchlo pridať</button>
            </div>

            <div id="pane-search" class="tab-pane active" style="display:block;">
                <div class="form-group" style="position:relative;">
                    <i class="ti ti-search" style="position:absolute; left:12px; top:50%; transform:translateY(-50%); color:#636b99;"></i>
                    <input type="text" class="form-control" placeholder="Meno, firma alebo email..." style="padding-left: 40px;" onkeyup="searchLeads(this.value)">
                </div>
                <div id="searchResults" style="max-height:280px; overflow-y:auto; padding-right:5px;">
                    <div style="text-align:center; color:#636b99; padding:20px; font-size:0.9rem;">Začnite písať pre vyhľadávanie...</div>
                </div>
            </div>

            <div id="pane-add" class="tab-pane" style="display:none;">
                ...
                <button id="qSubmitBtn" onclick="submitQuickAddLead()" class="btn btn-primary w-full mt-4" style="background:#6366f1; color:white; justify-content:center;">
