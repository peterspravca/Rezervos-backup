...
if (empty($token)) {
    ...
    $stmt = $conn->prepare("SELECT id FROM establishment_newsletter_subscribers WHERE unsubscribe_token = ?");
    $stmt->bind_param("s", $token);
    $stmt->execute();
    $row = $stmt->get_result()->fetch_assoc();
    if ($row) {
        $upd = $conn->prepare("UPDATE establishment_newsletter_subscribers SET confirmed = 0 WHERE id = ?");
...
<title>Odhlásenie z noviniek — Rezervos</title>
...
<div class="wrap">
    <div class="card">
        <?php if ($success): ?>
            ...
            <p>Novinky od tejto prevádzky vám už nebudeme posielať.</p>
        <?php else: ?>
