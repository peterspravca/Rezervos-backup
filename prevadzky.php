...
$conn->query("ALTER TABLE establishments ADD COLUMN IF NOT EXISTS top_expires_at DATETIME NULL DEFAULT NULL");
$conn->query("ALTER TABLE establishments ADD COLUMN IF NOT EXISTS sponsored_expires_at DATETIME NULL DEFAULT NULL");
$conn->query("ALTER TABLE establishments ADD COLUMN IF NOT EXISTS recommended_expires_at DATETIME NULL DEFAULT NULL");
...
// Radiusové vyhľadávanie lokality: namiesto presnej zhody textu nájdeme
// súradnice zadaného mesta/PSČ a rozšírime hľadanie na všetky obce v okruhu SEARCH_RADIUS_KM.
define('SEARCH_RADIUS_KM', 20);
$nearby_cities = [];

if (!empty($city)) {
    try {
        $clean_zip = str_replace(' ', '', $city);
        $geo_stmt = $conn->prepare("
            SELECT lat, lon FROM locations
            WHERE REPLACE(zip_code, ' ', '') = ? OR city_name = ?
            LIMIT 1
        ");
        $geo_stmt->bind_param("ss", $clean_zip, $city);
        $geo_stmt->execute();
        $geo_row = $geo_stmt->get_result()->fetch_assoc();

        if (!$geo_row) {
            $zip_prefix = $clean_zip . '%';
            $city_prefix = $city . '%';
            $geo_stmt2 = $conn->prepare("
                SELECT lat, lon FROM locations
                WHERE REPLACE(zip_code, ' ', '') LIKE ? OR city_name LIKE ?
                LIMIT 1
            ");
            $geo_stmt2->bind_param("ss", $zip_prefix, $city_prefix);
            $geo_stmt2->execute();
            $geo_row = $geo_stmt2->get_result()->fetch_assoc();
        }

        if ($geo_row && $geo_row['lat'] !== null) {
            $center_lat = (float)$geo_row['lat'];
            $center_lon = (float)$geo_row['lon'];

            $radius_km = SEARCH_RADIUS_KM;
            $radius_stmt = $conn->prepare("
                SELECT DISTINCT city_name,
                (6371 * acos(cos(radians(?)) * cos(radians(lat)) * cos(radians(lon) - radians(?)) + sin(radians(?)) * sin(radians(lat)))) AS distance
                FROM locations
                HAVING distance <= ?
            ");
            $radius_stmt->bind_param("dddd", $center_lat, $center_lon, $center_lat, $radius_km);
            $radius_stmt->execute();
            $radius_rows = $radius_stmt->get_result()->fetch_all(MYSQLI_ASSOC);
            foreach ($radius_rows as $r) {
                $nearby_cities[] = $r['city_name'];
            }
        }
    } catch (Exception $e) {
        $nearby_cities = [];
...
// Zostaví WHERE fragment pre filter lokality: text LIKE fallback + IN(...) pre obce v okruhu.
function build_city_filter_clause($city, $nearby_cities) {
    $clean_zip = str_replace(' ', '', $city);
    $city_like = "%" . $city . "%";
    $zip_like = "%" . $clean_zip . "%";

    $conditions = ["e.city LIKE ?", "e.address LIKE ?", "REPLACE(e.address, ' ', '') LIKE ?"];
    $params = [$city_like, $city_like, $zip_like];
    $types = "sss";

    if (!empty($nearby_cities)) {
        $placeholders = implode(',', array_fill(0, count($nearby_cities), '?'));
        $conditions[] = "e.city IN ($placeholders)";
        foreach ($nearby_cities as $nc) {
            $params[] = $nc;
            $types .= "s";
        }
    }
...
// Helper to determine the best matching image for an establishment
function get_establishment_image($est, $category_definitions) {
    ...
    // Keyword match
    if (stripos($cat, 'Barber') !== false || stripos($cat, 'Holi') !== false || stripos($name, 'Barber') !== false) {
        return 'assets/img/barber_shop.png';
    }
    if (stripos($cat, 'Necht') !== false || stripos($cat, 'Nails') !== false) {
        return 'assets/img/cat_nails.png';
    }
    if (stripos($cat, 'Masáž') !== false || stripos($cat, 'Spa') !== false || stripos($cat, 'Wellness') !== false) {
        return 'assets/img/cat_wellness.png';
    }
    if (stripos($cat, 'Pleť') !== false || stripos($cat, 'Kozmet') !== false || stripos($cat, 'Make-up') !== false) {
        return 'assets/img/cat_makeup.png';
    }
    if (stripos($cat, 'Tetov') !== false || stripos($cat, 'Tattoo') !== false) {
        return 'assets/img/cat_tattoo.png';
    }
    if (stripos($cat, 'Vrko') !== false || stripos($cat, 'Braids') !== false) {
        return 'assets/img/cat_braids.png';
    }
    
    // Fallback to exact category definition
    foreach ($category_definitions as $cat_key => $def) {
        if (!empty($def['img']) && (stripos($cat, $cat_key) !== false || stripos($cat_key, $cat) !== false)) {
            return $def['img'];
        }
    }
...
$per_page = 12; // 12 items per page (3 columns x 4 rows)
$offset = ($current_page - 1) * $per_page;

// Count total items matching filter
$count_sql = "SELECT COUNT(*) as total FROM establishments e WHERE e.status = 'active'";
$count_params = [];
$count_types = "";

if (!empty($category) && $category !== 'all' && $category !== 'Všetky') {
    if (strtolower($category) === 'last minute') {
        $count_sql .= " AND (e.id IN (SELECT DISTINCT establishment_id FROM last_minute_slots WHERE status = 'active') OR e.category = 'Last minute' OR e.category LIKE '%Last minute%')";
    } else {
        $count_sql .= " AND (e.category = ? OR e.category LIKE ?)";
...
if (!empty($search_query)) {
    $count_sql .= " AND (e.name LIKE ? OR e.city LIKE ? OR e.category LIKE ? OR e.description LIKE ? OR e.id IN (SELECT establishment_id FROM services WHERE name LIKE ?))";
    $like = "%" . $search_query . "%";
    $count_params[] = $like;
    $count_params[] = $like;
    $count_params[] = $like;
...
if (!empty($city)) {
    $city_filter = build_city_filter_clause($city, $nearby_cities);
    $count_sql .= $city_filter['sql'];
    $count_params = array_merge($count_params, $city_filter['params']);
    $count_types .= $city_filter['types'];
}

if (!empty($date) && preg_match('/^\d{4}-\d{2}-\d{2}$/', $date)) {
    $day_of_week = strtolower(date('D', strtotime($date)));
    $day_map = ['mon'=>'mon', 'tue'=>'tue', 'wed'=>'wed', 'thu'=>'thu', 'fri'=>'fri', 'sat'=>'sat', 'sun'=>'sun'];
    $day_key = $day_map[$day_of_week] ?? 'mon';
    $open_like = '%"' . $day_key . '":%';
    
    $count_sql .= " AND (
        e.id IN (SELECT establishment_id FROM last_minute_slots WHERE slot_date = ? AND status = 'active')
        OR e.opening_hours LIKE ?
        OR e.opening_hours IS NULL
    )";
    $count_params[] = $date;
    $count_params[] = $open_like;
    $count_types .= "ss";
}
...
// Query establishments from database for current page
$sql = "SELECT e.*,
        (SELECT MIN(price) FROM services WHERE establishment_id = e.id) as min_price,
        (SELECT COUNT(*) FROM bookings WHERE establishment_id = e.id) as total_bookings,
        (e.top_expires_at IS NOT NULL AND e.top_expires_at > NOW()) as is_top,
        (e.sponsored_expires_at IS NOT NULL AND e.sponsored_expires_at > NOW()) as is_sponsored,
        (
            (e.recommended_expires_at IS NOT NULL AND e.recommended_expires_at > NOW())
            OR (SELECT COUNT(*) FROM reviews r WHERE r.reviewee_id = e.user_id AND r.reviewee_type = 'establishment' AND r.rating = 5
                AND r.created_at >= DATE_FORMAT(NOW(), '%Y-%m-01')
                AND EXISTS (SELECT 1 FROM reviews r2 WHERE r2.booking_id = r.booking_id AND r2.reviewer_type = 'customer')) >= 100
        ) as is_recommended
...
if (!empty($category) && $category !== 'all' && $category !== 'Všetky') {
    if (strtolower($category) === 'last minute') {
        $sql .= " AND (e.id IN (SELECT DISTINCT establishment_id FROM last_minute_slots WHERE status = 'active') OR e.category = 'Last minute' OR e.category LIKE '%Last minute%')";
    } else {
...
if (!empty($search_query)) {
    $sql .= " AND (e.name LIKE ? OR e.city LIKE ? OR e.category LIKE ? OR e.description LIKE ? OR e.id IN (SELECT establishment_id FROM services WHERE name LIKE ?))";
    $like = "%" . $search_query . "%";
    $params[] = $like;
    $params[] = $like;
    $params[] = $like;
...
if (!empty($city)) {
    $city_filter = build_city_filter_clause($city, $nearby_cities);
    $sql .= $city_filter['sql'];
    $params = array_merge($params, $city_filter['params']);
    $types .= $city_filter['types'];
}
...
// Sorting logic — TOP > Sponzorované > Odporúčané majú vždy prednosť pred bežným triedením
$boost_order_prefix = " ORDER BY is_top DESC, is_sponsored DESC, is_recommended DESC, ";
if ($sort === 'price_asc') {
    $sql .= $boost_order_prefix . "min_price ASC, e.rating DESC";
} elseif ($sort === 'price_desc') {
    $sql .= $boost_order_prefix . "min_price DESC, e.rating DESC";
} elseif ($sort === 'newest') {
    $sql .= $boost_order_prefix . "e.id DESC";
} else {
    // Default: highest rating
    $sql .= $boost_order_prefix . "e.rating DESC, total_bookings DESC, e.id DESC";
}
...
$stmt->execute();
$establishments = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);

// Helper for generating page URL
function get_pagination_url($page_num, $category, $search_query, $city, $sort, $date = '') {
    $params = [];
    if (!empty($category) && $category !== 'all' && $category !== 'Všetky') $params['category'] = $category;
    if (!empty($search_query)) $params['q'] = $search_query;
    if (!empty($city)) $params['city'] = $city;
    if (!empty($date)) $params['date'] = $date;
    if (!empty($sort) && $sort !== 'rating') $params['sort'] = $sort;
    if ($page_num > 1) $params['page'] = $page_num;
    return 'prevadzky.php' . (!empty($params) ? '?' . http_build_query($params) : '');
}

// Page title and subtitle
$current_category_title = (!empty($category) && $category !== 'all' && $category !== 'Všetky') ? $category : 'Všetky prevádzky';
$current_category_desc = $category_definitions[$category]['desc'] ?? 'Vyhľadajte si najlepšie salóny a služby vo vašom okolí';
?>
<!DOCTYPE html>
...
<head>
    ...
    <title><?php echo htmlspecialchars($current_category_title); ?> | <?= BRAND_NAME ?></title>
    <meta name="description" content="<?php echo htmlspecialchars($current_category_desc); ?>">
    ...
    <style>
        ...
        *, *::before, *::after {
        ...
        .sidebar-logo img.rezervos-pv-logo,
        body.dark-mode .sidebar-logo img.rezervos-pv-logo { filter: none !important; }
        .rezervos-pv-text-html { font-size: 34px; font-weight: 800; letter-spacing: -0.5px; white-space: nowrap; }
        ...
        .sidebar-scroll-area::-webkit-scrollbar { display: none; width: 0; height: 0; }
        ...
        body.dark-mode .sidebar-scroll-indicator { background: #b08042; }
        ...
        /* Medzera pod GPS položkou len vtedy, keď pod ňou naozaj nasleduje ďalšia položka —
           inak (osamotená GPS položka) by vznikol nerovnaký okraj dole oproti hornej/bočným stranám. */
        ...
        .boost-badge .material-symbols-outlined {
            font-size: 14px;
        }

        .boost-badge.boost-top { background: #f59e0b; }
        .boost-badge.boost-sponsored { background: #8b5cf6; }
        .boost-badge.boost-recommended { background: #10b981; }
        ...
        /* Pätička (Footer) — štýly sú v assets/css/site-footer.css (zdieľané s index.php/inzercia.php) */
...
<body>
    ...
    <div class="app-layout sidebar-pinned" id="app-layout">
        ...
        <aside id="app-sidebar" class="app-sidebar">
            ...
            <div class="sidebar-section-title"><?php echo t('Kategórie'); ?></div>

            <nav class="sidebar-scroll-area">
                <!-- Všetky kategórie -->
                <a href="prevadzky.php<?php echo !empty($search_query) ? '?q='.urlencode($search_query) : ''; ?>"
                   class="category-nav-item <?php echo (empty($category) || $category === 'all' || $category === 'Všetky') ? 'active' : ''; ?>">
                    <span class="material-symbols-outlined nav-icon">grid_view</span>
                    <span><?php echo t('Všetky kategórie'); ?></span>
                </a>

                <a href="inzercia.php" class="category-nav-item">
                    <span class="material-symbols-outlined nav-icon">newspaper</span>
                    <span><?php echo t('Inzercia'); ?></span>
                </a>
                ...
                <?php foreach ($category_definitions as $cat_name => $cat_data): ?>
                    ...
                    $url = 'prevadzky.php?category=' . urlencode($cat_name);
                    if (!empty($search_query)) $url .= '&q=' . urlencode($search_query);
                    if (!empty($city)) $url .= '&city=' . urlencode($city);
                    if (!empty($sort) && $sort !== 'rating') $url .= '&sort=' . urlencode($sort);
                    ?>
                    <a href="<?php echo $url; ?>" class="category-nav-item <?php echo $is_active ? 'active' : ''; ?>">
                        <span class="material-symbols-outlined nav-icon"><?php echo $cat_data['icon']; ?></span>
                        <span><?php echo htmlspecialchars(t($cat_name)); ?></span>
                    </a>
                <?php endforeach; ?>
            </nav>
            <div class="sidebar-scroll-indicator" aria-hidden="true"></div>
        </aside>
        ...
        <div class="app-main-content">
            ...
            <!-- PAGE BODY (Centered Content with Filter & Grid) -->
            <div class="page-body">
                
                <!-- Search & Filter Panel (Aveino style) -->
                <div class="filter-panel">
                    <form method="GET" action="prevadzky.php" class="filter-form">
                        <?php if (!empty($category) && $category !== 'all' && $category !== 'Všetky'): ?>
                        ...
                        <div class="filter-input-wrapper">
                            ...
                            <input type="text" 
                                   name="q" 
                                   id="filterSearchService"
                                   class="filter-input" 
                                   placeholder="<?php echo htmlspecialchars(t('Akú službu hľadáte?')); ?>"
                                   value="<?php echo htmlspecialchars($search_query); ?>"
                                   autocomplete="off">
                            <div class="autocomplete-suggestions" id="filterServiceSuggestions"></div>
                        </div>

                        <!-- City Input with GPS Radar -->
                        <div class="filter-location-wrapper">
                            <span class="material-symbols-outlined input-icon">location_on</span>
                            <input type="text" 
                                   name="city" 
                                   id="filterSearchLocation"
                                   class="filter-input" 
                                   placeholder="<?php echo htmlspecialchars(t('Mesto alebo PSČ')); ?>"
                                   value="<?php echo htmlspecialchars($city); ?>"
                                   autocomplete="off">
                            <div class="autocomplete-suggestions" id="filterLocationSuggestions"></div>
                        </div>

                        <!-- Date / Calendar Filter Input -->
                        <div class="filter-date-wrapper" id="filterDateWrapper">
                            <span class="material-symbols-outlined input-icon" id="filterDateIcon">calendar_month</span>
                            <input type="text" 
                                   name="date" 
                                   id="filterSearchDate"
                                   class="filter-input filter-date-input" 
                                   placeholder="<?php echo htmlspecialchars(t('Vyberte voľný termín')); ?>"
                                   value="<?php echo htmlspecialchars($date); ?>"
                                   title="<?php echo htmlspecialchars(t('Vyberte voľný termín')); ?>"
                                   autocomplete="off">
                        </div>
                        ...
                        <button type="submit" class="btn-search">
                            ...
                            <span><?php echo t('Hľadať'); ?></span>
                        </button>
                    </form>

                    <!-- Active Filter Tags -->
                    <?php if (!empty($category) || !empty($search_query) || !empty($city) || !empty($date)): ?>
                        <div class="active-filter-tags">
                            <span style="font-size: 0.8rem; font-weight: 700; color: var(--text-secondary);"><?php echo t('Aktívne filtre:'); ?></span>

                            <?php if (!empty($category) && $category !== 'all' && $category !== 'Všetky'): ?>
                                <div class="filter-tag">
                                    <span><?php echo t('Kategória:'); ?> <strong><?php echo htmlspecialchars(t($category)); ?></strong></span>
                                    <a href="prevadzky.php?<?php echo http_build_query(array_filter(['q' => $search_query, 'city' => $city, 'date' => $date, 'sort' => $sort])); ?>" title="<?php echo htmlspecialchars(t('Zrušiť filter')); ?>">
                            ...
                            <?php if (!empty($search_query)): ?>
                                <div class="filter-tag">
                                    <span><?php echo t('Hľadanie:'); ?> <strong><?php echo htmlspecialchars($search_query); ?></strong></span>
                                    <a href="prevadzky.php?<?php echo http_build_query(array_filter(['category' => $category, 'city' => $city, 'date' => $date, 'sort' => $sort])); ?>" title="<?php echo htmlspecialchars(t('Zrušiť filter')); ?>">
                                        <span class="material-symbols-outlined" style="font-size: 14px;">close</span>
                                    </a>
                                </div>
                            <?php endif; ?>
                            ...
                            <?php if (!empty($city)): ?>
                                <div class="filter-tag">
                                    <span><?php echo t('Mesto:'); ?> <strong><?php echo htmlspecialchars($city); ?></strong></span>
                                    <a href="prevadzky.php?<?php echo http_build_query(array_filter(['category' => $category, 'q' => $search_query, 'date' => $date, 'sort' => $sort])); ?>" title="<?php echo htmlspecialchars(t('Zrušiť filter')); ?>">
                                        <span class="material-symbols-outlined" style="font-size: 14px;">close</span>
                                    </a>
                                </div>
                            <?php endif; ?>
                            ...
                            <?php if (!empty($date)): ?>
                                <div class="filter-tag">
                                    <span><?php echo t('Termín:'); ?> <strong><?php echo htmlspecialchars(date('d.m.Y', strtotime($date))); ?></strong></span>
                                    <a href="prevadzky.php?<?php echo http_build_query(array_filter(['category' => $category, 'q' => $search_query, 'city' => $city, 'sort' => $sort])); ?>" title="<?php echo htmlspecialchars(t('Zrušiť filter')); ?>">
                                        <span class="material-symbols-outlined" style="font-size: 14px;">close</span>
                                    </a>
                                </div>
                            <?php endif; ?>

                            <a href="prevadzky.php" style="font-size: 0.82rem; font-weight: 600; color: var(--primary-color); text-decoration: none; margin-left: 6px;">
                                <?php echo t('Vyčistiť všetky filtre'); ?>
                            </a>
                        </div>
                    <?php endif; ?>
                </div>
                ...
                <div class="breadcrumbs">
                    ...
                    <a href="prevadzky.php"><?php echo t('Výsledky vyhľadávania'); ?></a>
                    <?php if (!empty($category) && $category !== 'all' && $category !== 'Všetky'): ?>
                        <span class="material-symbols-outlined sep">chevron_right</span>
                ...
                <!-- Results Title, Counter and Sorting Dropdown -->
                <div class="page-title-row">
                    <h1 class="page-title">
                        <span class="material-symbols-outlined">grid_view</span>
                        <span><?php echo htmlspecialchars(t($current_category_title)); ?> (<?php echo $total_items; ?>)</span>
                    </h1>

                    <!-- Custom Sort Dropdown on the Right Side -->
                    <?php
                    $sort_definitions = [
                        'rating' => ['label' => 'Najlepšie hodnotené', 'icon' => 'star'],
                        'price_asc' => ['label' => 'Najlacnejšie', 'icon' => 'arrow_downward'],
                        'price_desc' => ['label' => 'Najdrahšie', 'icon' => 'arrow_upward'],
                        'newest' => ['label' => 'Najnovšie', 'icon' => 'schedule'],
                    ];
                    $active_sort = $sort_definitions[$sort] ?? $sort_definitions['rating'];
                    ?>
                    <div class="custom-dropdown" id="sort-dropdown">
                        <input type="hidden" name="sort" id="sort-input" value="<?php echo htmlspecialchars($sort); ?>">
                        <div class="custom-dropdown-trigger" id="sort-dropdown-trigger" tabindex="0" role="button" aria-haspopup="listbox" aria-expanded="false">
                            <div class="trigger-left">
                                <span class="material-symbols-outlined trigger-icon"><?php echo $active_sort['icon']; ?></span>
                                <span class="trigger-text"><?php echo htmlspecialchars(t($active_sort['label'])); ?></span>
                            </div>
                            <span class="material-symbols-outlined dropdown-arrow">expand_more</span>
                        </div>
                        <div class="custom-dropdown-menu" role="listbox" id="sort-dropdown-menu">
                            <?php foreach ($sort_definitions as $s_key => $s_data): ?>
                                <?php $is_selected = ($sort === $s_key || (empty($sort) && $s_key === 'rating')); ?>
                                <div class="custom-dropdown-item <?php echo $is_selected ? 'active' : ''; ?>" 
                                     data-value="<?php echo $s_key; ?>"
                                     role="option"
                                     aria-selected="<?php echo $is_selected ? 'true' : 'false'; ?>">
                                    <div class="item-left">
                                        <span class="material-symbols-outlined item-icon"><?php echo $s_data['icon']; ?></span>
                                        <span><?php echo htmlspecialchars(t($s_data['label'])); ?></span>
                                    </div>
                ...
                <?php if (empty($establishments)): ?>
                    <div class="empty-state">
                        ...
                        <h3><?php echo t('Nenašli sa žiadne prevádzky'); ?></h3>
                        <p><?php echo t('Pre zadané kritériá a kategóriu sme nenašli zodpovedajúce výsledky. Skúste zmeniť vyhľadávací dotaz alebo lokalitu.'); ?></p>
                        <a href="prevadzky.php" class="btn-search" style="text-decoration: none; margin-top: 8px;">
                            Zobraziť všetky prevádzky
                        </a>
                    </div>
                <?php else: ?>
                    <div class="establishments-grid">
                        <?php foreach ($establishments as $est): ?>
                            <?php 
                            $img = get_establishment_image($est, $category_definitions);
                            $rating = number_format($est['rating'] ?? 5.0, 1);
                            $est_url = !empty($est['custom_url']) ? '@' . rawurlencode($est['custom_url']) : 'profil.php?id=' . $est['id'];
                            $est['name'] = ct_get($conn, 'establishment', $est['id'], 'name', $est['name']);
                            ?>
                            <a href="<?php echo $est_url; ?>" class="listing-card">
                                <div class="listing-img-wrapper">
                                    <?php if (!empty($est['is_top'])): ?>
                                        <span class="boost-badge boost-top"><span class="material-symbols-outlined">workspace_premium</span> TOP</span>
                                    <?php elseif (!empty($est['is_sponsored'])): ?>
                                        <span class="boost-badge boost-sponsored"><span class="material-symbols-outlined">handshake</span> Sponzorované</span>
                                    <?php elseif (!empty($est['is_recommended'])): ?>
                                        <span class="boost-badge boost-recommended"><span class="material-symbols-outlined">thumb_up</span> Odporúčané</span>
                                    <?php endif; ?>
                                    <img src="<?php echo $img; ?>" alt="<?php echo htmlspecialchars($est['name']); ?>" onerror="this.onerror=null; this.src='assets/img/cat_hair.png';" loading="lazy">
                                    <button class="fav-btn" title="Uložiť medzi obľúbené" onclick="event.preventDefault(); event.stopPropagation(); toggleFavorite(this, <?php echo $est['id']; ?>);">
                                        <span class="material-symbols-outlined">favorite</span>
                                    </button>
                                </div>
                                <div class="listing-content">
                                    <h3 class="listing-title" title="<?php echo htmlspecialchars($est['name']); ?>">
                                        <?php echo htmlspecialchars($est['name']); ?>
                                    </h3>
                                    <p class="listing-salon">
                                        <span class="material-symbols-outlined">location_on</span>
                                        <span><?php echo htmlspecialchars($est['city'] ?? 'Bratislava'); ?></span>
                                    </p>
                                    <div class="listing-bottom">
                                        <span class="listing-price">
                                            <?php echo (isset($est['min_price']) && $est['min_price'] > 0) ? t('od') . " " . number_format($est['min_price'], 2) . " " . $currency : t('Cena neuvedená'); ?>
                                        </span>
                    ...
                    <?php if ($total_items > 0): ?>
                        <!-- Aveino-style Bottom Pagination & Navigation Bar -->
                        <div class="bottom-pagination-bar">
                            <!-- Left: Scroll to Top -->
                            <button type="button" class="btn-scroll-top" onclick="window.scrollTo({top: 0, behavior: 'smooth'})" title="<?php echo htmlspecialchars(t('Posunúť hore')); ?>">
                                <span class="material-symbols-outlined">arrow_upward</span>
                                <span><?php echo t('Hore'); ?></span>
                            </button>

                            <!-- Center: Load More / Next Page Pill -->
                            <?php if ($current_page < $total_pages): ?>
                                <?php 
                                $remaining = $total_items - ($current_page * $per_page);
                                $next_count = min($per_page, $remaining);
                                $next_page_url = get_pagination_url($current_page + 1, $category, $search_query, $city, $sort, $date);
                                ?>
                                <a href="<?php echo $next_page_url; ?>" class="btn-load-more-pill">
                                    <span><?php echo $next_count; ?> <?php echo t('ďalších...'); ?></span>
                                </a>
                            <?php else: ?>
                                <div style="font-size: 0.85rem; font-weight: 600; color: var(--text-secondary);">
                                    <?php echo t('Zobrazené všetky prevádzky'); ?> (<?php echo $total_items; ?>)
                                </div>
                            <?php endif; ?>
                            ...
                            <div class="pagination-nav">
                                ...
                                <?php if ($current_page > 1): ?>
                                    <a href="<?php echo get_pagination_url($current_page - 1, $category, $search_query, $city, $sort, $date); ?>" class="page-num" title="Predchádzajúca strana">
                                ...
                                if ($start_p > 1) {
                                    echo '<a href="' . get_pagination_url(1, $category, $search_query, $city, $sort, $date) . '" class="page-num">1</a>';
                                    if ($start_p > 2) {
                                ...
                                for ($p = $start_p; $p <= $end_p; $p++) {
                                    ...
                                    if ($is_cur) {
                                        echo '<span class="page-num active">' . $p . '</span>';
                                    } else {
                                        echo '<a href="' . get_pagination_url($p, $category, $search_query, $city, $sort, $date) . '" class="page-num">' . $p . '</a>';
                                    }
                                }
                                ...
                                if ($end_p < $total_pages) {
                                    ...
                                    echo '<a href="' . get_pagination_url($total_pages, $category, $search_query, $city, $sort) . '" class="page-num">' . $total_pages . '</a>';
                                }
                                ?>

                                <!-- Next Button -->
                                <?php if ($current_page < $total_pages): ?>
                                    <a href="<?php echo get_pagination_url($current_page + 1, $category, $search_query, $city, $sort); ?>" class="page-num" title="Ďalšia strana">
    ...
    <script>
        ...
        function updateSidebarState(pinned, persist = true) {
            if (pinned) {
                appLayout.classList.add('sidebar-pinned');
                appSidebar.classList.remove('hover-open');
                if (persist) localStorage.setItem('sidebar_pinned', 'true');
                ...
                if (persist) localStorage.setItem('sidebar_pinned', 'false');
                if (sidebarToggleIcon) {
        ...
        document.addEventListener('DOMContentLoaded', () => {
            if (window.innerWidth > 1024) {
                // Na PC je menu vždy otvorené pri načítaní — nepýtame sa localStorage, nič si nepamätáme.
                updateSidebarState(true, false);
            } else {
                updateSidebarState(false, false);
                // Krátke "mihnutie" menu, nech mobilný používateľ vie, že tam je — potom sa sama zavrie.
            ...
            if (sidebarScrollArea) {
                ...
                const updateScrollIndicator = () => {
                    ...
                    const indicatorHeight = Math.min(88, Math.max(64, sidebarScrollArea.clientHeight * 0.11));
                    const availableTravel = Math.max(0, sidebarScrollArea.clientHeight - indicatorHeight);
                    const progress = Math.min(1, Math.max(0, sidebarScrollArea.scrollTop / maxScroll));
                    scrollIndicator.style.height = indicatorHeight + 'px';
                    scrollIndicator.style.top = (sidebarScrollArea.offsetTop + progress * availableTravel) + 'px';
        ...
        if (filterSearchDate && typeof flatpickr !== 'undefined') {
            const fp = flatpickr(filterSearchDate, {
                ...
                showMonths: 1,
                defaultDate: <?php echo !empty($date) ? json_encode($date) : 'null'; ?>,
                onReady: function(selectedDates, dateStr, instance) {
                    if (instance.altInput) {
                        instance.altInput.placeholder = <?php echo json_encode(t('Vyberte voľný termín')); ?>;
